...
已实现 2A-2 外部用户映射和应用级授权基础。管理员可以维护同一租户内的外部用户映射，并通过停用保留历史关联；每个调用应用具有独立的 Agent 和工具白名单，未授权时默认为空。外部用户解析同时校验应用、映射和平台用户状态，并返回独立的被代表用户上下文，不继承平台管理身份。Invocation、幂等、用户与应用权限交集快照，以及允许和拒绝审计仍属于后续 2A-3。
