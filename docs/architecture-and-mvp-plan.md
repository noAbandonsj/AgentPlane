...
当前进度（2026-08-13）：已实现 2A-1 调用应用身份基础，包括管理员注册、查询、修改、停用、凭据签发与轮换，以及 Bearer 应用身份认证。完整密钥只在签发响应中返回，数据库只保存摘要；停用、撤销和过期凭据均不能继续认证。外部用户映射、应用级 Agent/工具授权、Invocation、幂等和拒绝审计仍属于后续 2A-2、2A-3。
