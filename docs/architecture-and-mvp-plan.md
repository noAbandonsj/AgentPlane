...
Invocation 和 TaskRun 的权限决策应演进为带 `schema_version` 的通用能力快照，资源项至少记录资源类型、资源标识、固定版本、允许动作和可选范围；现有 `effective_tool_keys` 在过渡期作为 Worker 的兼容字段保留。该扩展契约当前只作为后续设计约束记录，不在企业系统首次接入之前建设 Skill、知识库、通用 entitlement 表或相应管理页面。
...
当前进度（2026-08-13）：已实现 2A-1 调用应用身份基础，包括管理员注册、查询、修改、停用、凭据签发与轮换，以及 Bearer 应用身份认证。完整密钥只在签发响应中返回，数据库只保存摘要；停用、撤销和过期凭据均不能继续认证。
...
下一优先级进入 2C CRM 试点验收，验证真实应用凭据管理、重复请求、连续会话、权限拒绝、运行事件和最终结果。Skill、知识库和连接器权限在系统接入链路通过真实业务验收后分批实施，并复用前述版本化、动作化、最小权限交集和能力快照约定，不阻塞试点。
...
完成试点后，再按正式环境要求扩展 OIDC、可配置 RBAC/基础 ABAC、模型与凭据管理、租户配额、技能治理以及完整审批恢复，不将这些能力前置到 2A。
